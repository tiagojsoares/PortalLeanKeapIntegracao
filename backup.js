...
(async () => {
  ...
  for (let index = 0; index < meses.length; index++) {
    let mes = format(meses[index]);
    const result = await delay(2000, { value: mes });
    ObterContratos.then(dados => {
      token = dados.token;
      contratos = dados.contratos;


      // let contrato = factoryContrato(siteNome, mes, "2021", token, []);
      // constratos.push(contrato);
      // console.log(siteNome)
    })
    ...
    for (let i = 0; i < contratos.length; i++) {
      let SiteNome = contratos[i];
      // siteNome, month, year, token, days
      let contrato = factoryContrato(siteNome, mes, "2021", token, []);
      arrayContratos.push(contrato);
    }


    // criarContratosLogica(contratos, token);
    console.log("Rodando um mes:" + format(mes));
  }
...
function criarContratosLogica(contratos, token) {
  (async () => {
    const result = await delay(2000, { value: "mes" });
    console.log((token))
  })();
